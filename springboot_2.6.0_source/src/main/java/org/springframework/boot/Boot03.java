...
import org.springframework.boot.component.Bean13;
...
public class Boot03 {
    public static void main(String[] args) throws Exception {
        ...
        ApplicationEnvironment env = new ApplicationEnvironment(); // 这里只会涉及到systemProperties和systemEnvironment
...
//        ConfigDataEnvironmentPostProcessor configData = new ConfigDataEnvironmentPostProcessor(new DeferredLogs(), new DefaultBootstrapContext());
//        configData.postProcessEnvironment(env, springApplication);
        // EnvironmentPostProcessorApplicationListener监听器监听到该事件后，会调用监听器的回调方法,该回调方法中再调用EnvironmentPostProcessor后处理器的回调方法
        EventPublishingRunListener eventPublishingRunListener = new EventPublishingRunListener(springApplication, args);
        eventPublishingRunListener.environmentPrepared(new DefaultBootstrapContext(),env);
